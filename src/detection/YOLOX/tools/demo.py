...
class Predictor(object):
    ...
    def visual(self, output, img_info, cls_conf=0.35):
        ...
        bboxes = output[:, 0:4]; path1 = "./Projects/2021/Detection/GT/";
		# Intersection over Union (IoU): for Track E: 0.x, Track H: 0.x, and Track Q: 0.x → overall: 0.x
        # preprocessing: resize
        path2 = "./Projects/2021/Detection/Pred/";
        bboxes /= ratio; print (bboxes); 
        print (path1+img_info['file_name'][:-4]+".npy"); 
        print ("Debug 1.0");
        #gt = np.load(path1+img_info['file_name'][:-4]+".npy", allow_pickle=True); 
        print ("Debug 2.0");
        boxes = bboxes.detach().cpu().numpy(); #print (img_info);
        print (boxes);
        print ("Debug 3.0");
        #binary = np.zeros((480,640), 'uint8');
        print ("Debug 4.0");
        
        f = open(path2+img_info['file_name'][:-4]+".csv", "w")
            
        #f.close()
        cls = output[:, 6]
        scores = output[:, 4] * output[:, 5]
        new_cls_names = ["drain"];
        vis_res = vis(img, bboxes, scores, cls, cls_conf, new_cls_names) #self.cls_names)

        iter1 = 0;
        for box in boxes:
            print (box)
            # Create a Rectangle patch
            xmin = int(round(box[1])); xmax = int(round(box[3]));
            ymin = int(round(box[0])); ymax = int(round(box[2]));
            #binary_img[xmin:xmax,ymin:ymax] = 255 # fill
            print (xmin, xmax, ymin, ymax)
            if(scores[iter1] > cls_conf):
...
def main(exp, args):
    ...
    exp.output_dir = "./Projects/2021/Detection/YOLOX/data/"
